 ...
 * argon2 are not available on Workers — the runtime ships no native module
 * host for them — so PBKDF2-SHA256 with a high iteration count is the
 * standard substitute in this environment.
 */

// OWASP's 2023 guidance for PBKDF2-SHA256 is >=600,000 iterations, but the
// task spec pins this at 210,000 (OWASP's pre-2023 recommendation) to bound
// per-request CPU time on Workers. Revisit if Workers CPU limits change.
