...
describe("session", () => {
  ...
  describe("createSession", () => {
    ...
    it("mints distinct ids across calls (never a counter or anything guessable)", async () => {
      const a = await createSession(store, "user-1", T0);
      const b = await createSession(store, "user-1", T0);
