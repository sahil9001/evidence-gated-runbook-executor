...
describe("missingSources", () => {
  it("names allowed sources that contributed zero cards", () => {
    const packet = buildPacket({
      id: "p", incidentId: "i", runbookId: "r",
      cards: [card({ source: "logs" })],
      builtAt: "2026-08-25T02:01:00.000Z"
    });
    expect(missingSources(packet, ["logs", "metrics", "deploys"])).toEqual(["metrics", "deploys"]);
  });

  it("returns an empty array when every allowed source has a card", () => {
    const packet = buildPacket({
      id: "p", incidentId: "i", runbookId: "r",
  ...
  it("returns every allowed source for an empty packet", () => {
    ...
    expect(missingSources(packet, ["logs", "metrics"])).toEqual(["logs", "metrics"]);
  });
});
