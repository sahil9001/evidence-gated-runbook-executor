...
authRoutes.post("/auth/register", async (c) => {
  ...
  try {
    ...
    // Defense in depth against the same TOCTOU race B1 closed for approval
    // claims: two concurrent registrations can both pass the check above.
    // `users.email` is UNIQUE at the D1 layer, so the loser's INSERT throws
    // here instead of silently succeeding or corrupting state.
...
authRoutes.post("/auth/login", async (c) => {
  ...
  // "Invalid email" and "wrong password" produce the exact same response,
  // in the exact same amount of time, so neither the body nor the timing
  // reveals which case occurred.
  const passwordOk = await verifyPassword(password, user?.passwordHash ?? DUMMY_PASSWORD_HASH, user?.salt ?? DUMMY_PASSWORD_SALT);
