 ...
 * or malformed on-disk row directly (the C2 / M9 defence-in-depth tests in
...
export function runStoreConformance(name: string, makeStore: () => Promise<Store> | Store): void {
  describe(`${name} store conformance`, () => {
    ...
    describe("packets", () => {
      ...
      it("returns the packet with the latest builtAt, not the first one saved (M1)", async () => {
        const run = makeRun("run-m1", { incidentId: "inc-m1" });
        await store.createRun(run);

        // Saved out of chronological order, and with ids that would sort
        // the wrong way — the old `ORDER BY id` bug would return whichever
        // packet had the lexicographically-first UUID, not the newest one.
