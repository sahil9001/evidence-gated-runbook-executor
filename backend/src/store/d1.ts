...
export function createD1Store(db: D1Database): Store {
  return {
    ...
    async getPacketByIncident(incidentId: string): Promise<EvidencePacket | null> {
      // M1: ordered by `built_at DESC`, not by the packet's UUID primary
      // key — a UUID has no relationship to recency, so the old `ORDER BY
      // id` returned an arbitrary packet whenever an incident had more than
      // one.
      const record = await db
